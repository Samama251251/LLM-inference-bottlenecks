...
## Current compute (Vast.ai RTX 4060 Ti box)

We are no longer on Colab. Phase 1 runs on a rented Vast.ai box, see `docs/vastai.md` for the full record. Where this and any older Colab references disagree on hardware, the Vast.ai box wins. The earlier RTX 3060 12GB results in `docs/baseline-hf-results.md`, `docs/baseline-vllm-results.md`, and the committed OOM plot were run by a collaborator on a different 3060 box; we keep them on purpose as a cross-hardware comparison baseline (Ampere 12GB vs Ada 8GB). Do not relabel them as 4060 Ti numbers.

- GPU: one NVIDIA RTX 4060 Ti, 8GB, on-demand, about $0.095/hr. Ada Lovelace, compute capability 8.9 (sm_89). ~22 fp32 TFLOPS, ~288 GB/s memory bandwidth (vast.ai lists 233 GB/s; confirm on the box). Instance 42824740, located in Canada.
- Stack: Vast.ai image `vastai/pytorch_cuda-12.4.1-auto/jupyter`, Max CUDA 12.4, torch preinstalled for CUDA 12.4 (verify exact torch version on the box, it is NOT the old NGC torch 2.12+cu130). transformers, vLLM, accelerate are NOT preinstalled. HuggingFace and GitHub are both reachable from the box.
- Host: 11th Gen i7-11700 (16 vCPU), 64GB RAM, 32GB disk. Disk is small, so be deliberate about the weights cache.
...
- Two environments on purpose. Environment A (HF baseline) uses the preinstalled torch, just add transformers + accelerate. Environment B (vLLM) is a fresh venv at `/workspace/vllm-env` where vLLM brings its own matching torch for CUDA 12.4 (uv recommended: `uv pip install vllm --torch-backend=cu124`). Do NOT `pip install vllm` into Environment A: it would clobber the preinstalled torch with vLLM's bundled build. CUDA 12.4 is vLLM's mainstream target, so the wheel install is lower-friction than the old cu130 stack was.
- The sm_89 card resolves the old T4 attention-backend worry: FlashAttention and vLLM's good kernels support this card (sm_89 also adds fp8), so no fallback backend.

## Hard technical constraints (these change your decisions)

- Target fp16. NOT bf16. The 4060 Ti supports bf16 (and fp8), but we still target fp16 for comparability with the later NUST HPC V100/T4 runs (which do not support bf16) and with the 3060 baseline, and it does not change the KV-cache OOM math (fp16 and bf16 are both 2 bytes per element). Never default to bf16.
- Single GPU, the one RTX 4060 Ti, 8GB. If a multi-GPU runtime ever appears, pin to one device so the measurement stays clean single-GPU.
...
- The small GPU is intentional for the OOM experiment. We WANT to hit the KV-cache wall at a realistic context length, so do not suggest a bigger GPU to "avoid" the crash. The crash is the experiment. On 8GB the cliff comes notably sooner than on the 12GB 3060, which makes the sweep faster and cheaper and gives a clean two-card comparison of where the wall lands.
